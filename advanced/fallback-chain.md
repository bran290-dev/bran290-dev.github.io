...
여러 카테고리에 동일한 API 설정을 한 번에 등록하는 기능입니다.

### 사용 방법

1. API 설정 화면에서 "일괄 등록" 버튼 클릭
2. 등록할 모델과 API 키 선택
3. 확인 클릭

### 동작

- **전처리**, **번역**, **유틸리티** 3개 카테고리에 동일한 설정을 한 번에 등록
- 이미 등록된 설정은 자동으로 건너뜀 (중복 방지)
- 등록 결과: "N개 항목이 등록되었습니다" 또는 중복 시 "N개 항목 등록됨 (M개 중복 스킵)"

> **팁**: 처음 설정할 때 일괄 등록을 사용하면 빠르게 모든 필수 카테고리를 구성할 수 있습니다.
...
| 모델 | 특징 |
|------|------|
| **Gemini 2.5 Flash** | 빠르고 경제적 (앱 기본값) |
| **Gemini 2.5 Pro** | 높은 품질 |
| **Gemini 2.5 Flash-Lite** | 매우 빠름, 저비용 |
| **Gemini 3.1 Pro Preview** | 최신 (실험적, 유료 전용) |
| **Gemini 3 Flash Preview** | 최신 (실험적) |
| **Gemini 2.0 Flash-Lite** | 가벼움 |
| **Gemma 3 27B** | 오픈소스 (완전 무료) |

**Grok (xAI)**

| 모델 | 특징 |
|------|------|
| **Grok 4.1 Fast** | 가성비 우수, 빠른 속도 |
...
|------|------|
| **GLM-4.7** | 고품질 |
| **GLM-4.7 Flash** | 무료 기본 모델 |
| **GLM-4.7 FlashX** | 경량 고속 |

**OpenAI Compatible**

OpenAI API 호환 서비스(Deepseek, Mistral, Ollama 등)를 사용하는 경우, 모델 ID를 직접 입력하여 체인에 추가할 수 있습니다.
...
```
1. Gemini 3 Flash Preview (최신, 최고 품질)
2. Gemini 2.5 Pro (고품질)
...
```
1. Grok 4.1 Fast (빠르고 저렴)
2. Gemini 2.5 Flash (무료 티어 백업)
3. GLM-4.7 Flash (무료 백업)
```

**이런 분께 추천**:
- 여러 프로바이더를 활용하여 안정성을 극대화하고 싶은 분
- 무료 모델을 조합하여 비용을 절약하고 싶은 분
